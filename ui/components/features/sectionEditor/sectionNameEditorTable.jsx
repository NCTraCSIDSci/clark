...
class SectionNameEditorTable extends React.Component {
  constructor(props) {
    super(props);
    // callbacks={this.props.callbacks}
    // data={this.props.sectionNameData}
    ...
    this.state = {
      columnDefs: [
        ...
        {field: "useStr", headerName: "In Use", width: 50, editable: true, cellEditor: "select", cellEditorParams: {values: ['Yes', 'No']}, onCellValueChanged: this.onUseChange},
      ],
  ...
  deleteRow() {
    ...
    if (newIdx <= (this.props.data.length - 1)) {
      this.props.callbacks.removeSectionName(newIdx);
  ...
  moveRowUp() {
    ...
    this.props.callbacks.moveSectionName(rowIdx, newIdx);
    this.api.setFocusedCell(newIdx, cellInfo.column.colId, cellInfo.column.floating);
    this.api.ensureIndexVisible(newIdx);
  }
  ...
  insertRow() {
    ...
    this.props.callbacks.newSectionName(newIdx);
  ...
  handleRowUpdate(rowIdx, newRow) {
    ...
    this.props.callbacks.editSectionName(rowIdx, newRow);
  }
  ...
  render() {
    return (
      <div>
        ...
        <div className="row" />
        <div className="ag-fresh" style={{height: 200, width: "100%"}}>
          <AgGridReact
...
export default SectionNameEditorTable;
