...
class KeywordEditorTable extends React.Component {
  constructor(props) {
    super(props);
    // patient = patientName
    ...
    this.state = {
      columnDefs: [
        {field: "name", headerName: "Name", width: 75, editable: true, cellEditor: 'text', onCellValueChanged: this.onNameChange, cellStyle: this.cellStyleIsValidName},
        {field: "regex", headerName: "Reg. Exp", width: 75, editable: true, cellEditor: 'text', onCellValueChanged: this.onRegexChange, cellStyle: this.cellStyleIsValidRegex},
      ],
  ...
  deleteRow() {
    ...
    if (newIdx <= (this.props.data.length - 1)) {
      this.props.callbacks.removeKeyword(newIdx);
  ...
  moveRowUp() {
    ...
    this.props.callbacks.moveKeyword(rowIdx, newIdx);
    this.api.setFocusedCell(newIdx, cellInfo.column.colId, cellInfo.column.floating);
    this.api.ensureIndexVisible(newIdx);
  }
  ...
  insertRow() {
    ...
    this.props.callbacks.newKeyword(newIdx);
  ...
  handleRowUpdate(rowIdx, newRow) {
    ...
    this.props.callbacks.editKeyword(rowIdx, newRow);
  }

  rowStyleIsValid(params) {
    let out = {};
    if (!params.data.regexIsValid || !params.data.nameIsValid) {
      // out = 'table-row-error';
  ...
  cellStyleIsValidName(params) {
    let out = {};
    if (!params.data.nameIsValid) {
      // out = 'table-row-error';
      out = {color: "#fff",
  ...
  cellStyleIsValidRegex(params) {
    let out = {};
    if (!params.data.regexIsValid) {
      // out = 'table-row-error';
      out = {color: "#f77189",
             border: "2px solid #d9534f",
             margin: 0,
...
export default KeywordEditorTable;
